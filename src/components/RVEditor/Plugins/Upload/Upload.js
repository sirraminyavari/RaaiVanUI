...
import InsertTag from '../../Util/InsertTag';

const { RVDic, GlobalUtilities, Base64 } = window;

const Upload = ({ ownerId, ownerType }) => {
  let uploader = null;
  let container = null;
  ...
  return CreatePlugin({
    onInit: function ({ editor: _editor }) {
      editor = _editor;
      console.log('RVUpload Plugin Loaded! :)');
    },
    button: {
      name: 'rv_upload',
      ...
      onClick: function (view, element) {
        const { DocsAPI } = window;

        if (!DocsAPI) return;
        else if (uploader) return uploader.browse();
        ...
        GlobalUtilities.uploader(
          container,
          {
            ...
            OnUpload: function (file, result) {
              if (result.success) {
                var extension = Base64.decode(
                  result.AttachedFile.Extension
                ).toLowerCase();
                var fileName = Base64.decode(result.AttachedFile.FileName);

                InsertTag(editor, {
                  ID: result.AttachedFile.FileID,
                  Type: 'File',
